...
Completion capacity can be selected independently with
`--completion-embedding`, `--completion-hidden`, and `--completion-layers`.
Defaults preserve the original 96/192/2 GRU (473,298 parameters). The larger
160/384/3 configuration has 2,562,322 parameters and retains causal state
across source chunks. Train it independently with `--tasks completion`; its
export includes dimensions and parameter count in `completionConfig`.

```bash
.venv310/bin/python -m training.syl2_trainer --manifest sources.jsonl \
  --tasks completion --completion-embedding 160 --completion-hidden 384 \
  --completion-layers 3 --cpu-threads 1 --epochs 10 \
  --output-dir runs/completion-large --ledger runs/completion-large-source-use.jsonl
```

Use a fresh output directory for each experiment. Epoch metrics are appended
to `epochs.jsonl` there as well as printed to stderr. The completion metric
currently named `precision` is next-byte top-1 accuracy, not whole-word
completion acceptance. The earlier 48.54% result was still improving at its
10-epoch cap; it does not establish a model-capacity ceiling. Comparisons on
the small local corpus are exploratory, not broad multilingual validation.

Completed capacity experiment: the larger model reached **55.67%** after 10
CPU epochs, versus **49.43%** for a fresh small-model control using identical
source hashes and the same held-out Python file. See the
[experiment report](runs/completion-large-cpu/EXPERIMENT.md) and
[larger completion binary](runs/completion-large-cpu/next-word.model.bin).

CPU sizing benchmark: the 17M-parameter configuration generated 16 bytes in
about 41 ms and 32 bytes in about 80 ms using four CPU threads. The 31M model
took about 82 ms and 166 ms; the 65M model took about 190 ms for 16 bytes.
For complete identifiers, 17M is the practical target for the 10–150 ms
budget. The 65M model is about 261 MB in float32 and leaves the budget after
only eight generated bytes. Reproduce the sweep with
`training.benchmark_completion`; results are in
[benchmark.json](runs/completion-cpu-scaling/benchmark.json) and the longer
[confirmation report](runs/completion-cpu-scaling/confirmation.json).
